...
var addScheduledJob = function(pipe){
	...
	sdpLog.info("Add new scheduled job: %d:%d", rule.hour, rule.minute);
	var job = schedule.scheduleJob(rule, function(){
		if ( global.currentRun ){
			sdpLog.error("Unable to execute a schedule run for pipe %s because a run is already in progress", pipe._id);
			return;
		}
...
var removeScheduledJob = function( pipe ){
	if ( jobs.hasOwnProperty( pipe._id )){
		sdpLog.info("Removing scheduled job for pipe id " + pipe._id);
		//cancel existing job
...
.addView( 
	"all_pipes",
	{
		map: function(doc){
			if ( doc.type === "pipe" ){
				emit( doc._id, {connectorId: doc.connectorId, pipeName: doc.name} );
			}
		}
	}, 2 //Version
)
...
.addView(
	'run_status_view',
	{
		map: function(doc){
      		if ( doc.pipeId && doc.type === 'run' ) {
      	  		emit( doc._rev, {pipeId : doc.pipeId, connectorId: doc.connectorId, status : doc.status} );
			}
      	}
	}, 1
);
				...
				pipeDb.upsert( pipe._id, function( storedPipe ){
					if ( storedPipe && storedPipe.hasOwnProperty("run") ){
						...
						pipeDb.upsert( storedPipe.run, function( doc ){
							...
							if ( err ){
								sdpLog.error( "Unable to save run document: " + err );
							}
						});						
					}
					...
					if ( err ){
						sdpLog.error("Unable to break reference to stale run " + err );
					}
				});
			}
				...
				if(_.every(body.rows, function (row) {
					if(row.value.pipeId == pipe._id) {
						if(row.value.status == 'FINISHED' || row.value.status == 'STOPPED' || row.value.status == 'ERROR') {
							...
							return true;
						} else if((row.value.status == 'NOT_STARTED') || (row.value.status == 'RUNNING')) {
