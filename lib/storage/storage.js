...
var sdpLog = require('../logging/sdpLogger').getLogger('sdp_common');
...
if (!cloudantService) {
    sdpLog.error("No instance of the Cloudant service is bound to this application.");
    throw new Error( "No instance of the Cloudant service is bound to this application." );
}

sdpLog.info("Using cloudant service \"" + cloudantService.name +"\"");
...
function storage( serviceDbName, viewsManager ){
					...
					if ( err ){
						...
						if ( viewsManager ){
							...
							_.forEach( viewsManager, function( manager ){
								...
								sdpLog.info("Creating design doc " + manager.designName + " in database " +  serviceDbName + ".");

								var designDoc = {};
								if (manager.viewDefs && manager.viewDefs.length > 0) {
									designDoc['views'] = manager.getViewsJson()
								}
								if (manager.indexDefs && manager.indexDefs.length > 0) {
									designDoc['indexes'] = manager.getIndexesJson()
								}
						...
						self.storageDb.get( manager.designName, function( err, body){
							...
							if ( err ){
								...
								_.forEach( manager.viewDefs, function( viewDef ){
									if (views && views.hasOwnProperty( viewDef.viewName) ){
								...
								if (! recreateDesignDoc) {
									// check for updated indexes
									var indexes = body.indexes;
									_.forEach( manager.indexDefs, function( indexDef ){
										if (indexes && indexes.hasOwnProperty( indexDef.indexName) ){
											var version = indexes[indexDef.indexName ].version || 0;
											if ( version < indexDef.version ){
												recreateDesignDoc = true;
											}
										}else{
											recreateDesignDoc = true;
	...
	when.promise( this.initCloudant )
		.then( function() {
			sdpLog.info("Cloudant database %s successfully initialized", self.serviceDbName);
			self.emit("cloudant_ready");
		})
		.otherwise( function(err) {
			sdpLog.error("Error initializing cloudant database " + self.serviceDbName + ": " + err);
	...
	this.upsert = function( docId, callback, done ){
		this.run( function( err, db ){
			...
			var insert = function( body ){
				...
				if ( body ){
					...
					db.insert( body, body._id, function( err, data ){
						...
						if ( err ){
							sdpLog.error( "Error while saving doc: " + err + ". Doc id: " + body._id + " Doc rev: " + body._rev);
							return done(err);
						}
			...
			if ( this.savesInProgress.hasOwnProperty( docId ) ){
				...
				//sdpLog.info("Deferring save of document: " + docId + " :::: " + this.savesInProgress[docId]);
	...
	this.deleteDocsFromView = function( designName, viewName, callback ){
		//sdpLog.info("Deleting all docs in view: " + designName + " / " + viewName );
		this.run( function( err, db ){
			if ( err ){
				return callback( err );
			}
...
function viewsManager( designName ){
	...
	this.addIndex = function( indexName, indexCode, version ){
		indexCode.version = version || 1;
		this.indexDefs.push( {
			indexName: indexName,
			indexCode: indexCode,
			version: indexCode.version
		});
		return this;
	}
	...
	this.getIndexesJson = function(){
		var json = {};
		_.forEach( this.indexDefs, function( indexDef ){
			json[indexDef.indexName ] = indexDef.indexCode;
		});
		return json;
	}
};
