			...
			var processTable = function( logger, table, targetDb, callback ){
				...
				thisConnector.fetchRecords( table, 
											...
											function( message ){
												...
												if ( message ){													
													logger.warn( 'Fetch operation for ' + table.name + ' returned message: ' + message );
													stats.errors.push( {message: message} );
													pipeRunStats.addTableStats( stats );
			...
			async.map( tables, function( table, callback ){
				...
				if ( _.isArray( statsArray ) ){
					_.forEach( statsArray, function(stats){
						if ( stats ){
							if(stats.numRecords) {
								totalCopied = totalCopied + stats.numRecords;
							}					
				...
				var message = require("util").format( "Copied %d records from %s to Cloudant", totalCopied, thisConnector.getLabel());
				logger.info( message );
				this.setStepMessage( message );
				return callback( err );
			}.bind(this));
